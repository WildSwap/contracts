...
contract MasterChef is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accWildyPerShare;   // Accumulated EGGs per share, times 1e12. See below.
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 wildyReward = multiplier.mul(wildyPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        wildy.mint(devaddr, wildyReward.div(10));
    ...
    function updateEmissionRate(uint256 _wildyPerBlock) public onlyOwner {
        massUpdatePools();
        wildyPerBlock = _wildyPerBlock;
    }
}
